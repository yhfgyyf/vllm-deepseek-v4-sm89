...
def render_reasoning_effort(
    ...
    The prefix is emitted only for thinking mode at index zero.
    """
    if effort is None:
        effort = DEFAULT_REASONING_EFFORT
    assert (type(effort) is int and 1 <= effort <= 100) or (
        effort in REASONING_EFFORT_MAPPINGS
    ), (
        "Invalid reasoning effort for deepseek_v41: "
        f"{effort}, should be int within [1,100] or "
        f"{list(REASONING_EFFORT_MAPPINGS)}"
    )
...
def render_message(
    ...
    if role == "system":
        ...
        raise NotImplementedError(
            "deepseek_v41 merges tool messages into user; please preprocess "
            "with merge_tool_messages()"
        )
        ...
        # A response to a task is emitted without thinking.
