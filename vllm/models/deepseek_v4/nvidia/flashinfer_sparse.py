...
def _flashinfer_sparse_mla_support_error(
    ...
    if capability == DeviceCapability(8, 9):
        if not has_flashinfer_sparse_mla_sm89():
            return "SM89 requires FlashInfer with native sparse MLA SM89 JIT support"
    ...
    def _forward(
        ...
        if num_prefill_tokens > 0:
            ...
            flashinfer_trtllm_batch_decode_sparse_mla_dsv4(
                ...
                swa_kv_cache=swa_k_cache,
                workspace_buffer=workspace,
                sparse_indices=sparse_indices[num_decode_tokens:num_tokens].contiguous(),
                compressed_kv_cache=compressed_kv_cache,
                sparse_topk_lens=sparse_topk_lens[num_decode_tokens:num_tokens].contiguous(),
...
class DeepseekV4FlashInferSM120Attention(DeepseekV4Attention):
    ...
    def _forward_decode(
        ...
        flashinfer_trtllm_batch_decode_sparse_mla_dsv4(
            ...
            extra_sparse_indices=(None if extra_sparse_indices is None else extra_sparse_indices.contiguous()),
