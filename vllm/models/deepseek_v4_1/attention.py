...
class DeepseekV41SWACache(DeepseekV4SWACache):
...
def _resolve_dsv41_kv_cache_dtype(
    kv_cache_dtype: str,
    cache_config: CacheConfig | None,
) -> tuple[str, torch.dtype]:
    """Resolve the native mixed-cache allocation dtype."""
    if kv_cache_dtype == "auto":
        kv_cache_dtype = "fp8"
    if not kv_cache_dtype.startswith("fp8"):
        raise ValueError(
            "DeepSeek V4.1 native attention only supports its mixed FP8/FP4 "
            f"KV cache, got {kv_cache_dtype}. Set `--kv-cache-dtype fp8`."
        )
    if kv_cache_dtype != "fp8_ds_mla":
        if cache_config is not None:
            cache_config.cache_dtype = "fp8_ds_mla"
        kv_cache_dtype = "fp8_ds_mla"
        logger.info_once("Using DeepSeek V4.1's native mixed KV cache format.")
    return kv_cache_dtype, torch.uint8


class DeepseekV4Attention(nn.Module, AttentionLayerBase, ABC):
    """DeepSeek V4.1 attention layer with a native platform implementation."""
    ...
    def __init__(
            ...
            if self.is_kv_source:
                index_k_cache = DeepseekV4IndexerCache(
                    head_dim=_indexer_k_cache_head_dim(
                        config.index_head_dim, dsv41_indexer_uses_fp4(vllm_config)
                    ),
                    dtype=torch.uint8,
        ...
        self.kv_cache_dtype, self.kv_cache_torch_dtype = _resolve_dsv41_kv_cache_dtype(
            cache_config.cache_dtype, cache_config
        )

        self.swa_cache_layer = DeepseekV41SWACache(
            head_dim=self.head_dim,
    ...
    def forward(
        ...
        # Allocate at the backend head count and slice if a backend pads heads.
    ...
    def _prepare_and_attn(
        ...
        compressor = None if self.ced_global_kv_prebuilt else self.compressor
    ...
    def _run_parallel_input_projections(
        ...
        if self.compressor is not None and not self.ced_global_kv_prebuilt:
    ...
    def build_ced_global_kv(
        self, hidden_states: torch.Tensor, positions: torch.Tensor
    ) -> None:
        """Publish full encoder context without decoder Q, SWA, or index queries."""
        compressor = self.compressor
        if compressor is None or self.indexer is None or not self.indexer.owns_k:
            raise ValueError("CED global-KV preparation requires a KV-source layer")
        kv_score = torch.mm(
            hidden_states,
            compressor.fused_wkv_wgate.weight.T,
            out_dtype=torch.float32,
    ...
    def _fused_qnorm_rope_kv_insert(
        ...
        return fused_q_rope_swa_insert(
            q,
            kv,
            positions,
            cos_sin_cache,
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec | None:
        ...
        if not self.is_kv_source:
            return None
        # The global mixed FP4 cache uses 288-byte rows aligned to 32 bytes.
...
class DeepseekV4Indexer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.compress_ratio = compress_ratio
        self.owns_k = owns_k
        self.use_fp4_kv = dsv41_indexer_uses_fp4(vllm_config)
        logger.info_once(
        ...
        self.quant_block_size = MXFP4_BLOCK_SIZE
        self.topk_indices_buffer = topk_indices_buffer
