...
from .quant_utils import _ceil_log2_scale
...
def _q_kv_norm_quant_kernel(
    ...
    if tl.program_id(1) == 0:
        ...
        if row < num_tokens:
            ...
            amax = tl.maximum(tl.max(tl.abs(grouped), 1), 1.0e-4)
            normalized = amax * (1.0 / 448.0)
            sf = _ceil_log2_scale(normalized) + 127
            inv_bits = (254 - sf) << 23
            ...
            sf = tl.full((BLOCK // 32,), 0, tl.int32)
        if ROW_MAJOR_SCALES:
            offsets = row * (Q_SIZE // 32) + groups
            tl.store(scales + offsets, sf, (row < num_tokens) & (groups < Q_SIZE // 32))
        else:
...
def fused_q_kv_rmsnorm_quant(
    ...
    """Normalize Q/KV and produce the architecture's native MXFP8 activation ABI."""
    ...
    kv_size = kv.shape[1]
    if not qr.is_cuda or qr.dtype != torch.bfloat16 or kv.dtype != qr.dtype:
        raise ValueError("V4.1 query normalization requires CUDA BF16 inputs")
    if q_weight.shape != (q_size,) or kv_weight.shape != (kv_size,):
        raise ValueError("Normalization weights must match projection dimensions")
    if any(t.device != qr.device for t in (kv, q_weight, kv_weight)):
        raise ValueError("All query normalization tensors must share a CUDA device")
    row_major_scales = current_platform.is_device_capability(89)
    if not row_major_scales and not current_platform.is_device_capability_family(120):
        raise NotImplementedError(
            "V4.1 native query quantization requires SM89 or SM120"
        )
    ...
    padded_tokens = tokens if row_major_scales else triton.cdiv(tokens, 128) * 128
    padded_groups = triton.cdiv(q_size // 32, 4) * 4
    scale_shape = (
        (tokens, q_size // 32) if row_major_scales else (padded_tokens * padded_groups,)
    )
    scales = torch.empty(scale_shape, dtype=torch.uint8, device=qr.device)
    if tokens:
...
def can_fuse_query_quant(linears: list[torch.nn.Module]) -> bool:
    ...
    consumers = (
        ...
        else (FlashInferCutedslMxfp8LinearKernel, FlashInferCutlassMxfp8LinearKernel)
    )
    return all(
